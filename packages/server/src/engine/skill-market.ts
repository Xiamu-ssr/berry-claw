...
//     to surface stats for each hit. Skills not in that explore cache
...
export class ClawHubSource implements SkillSource {
  ...
  private static EXPLORE_LIMIT = 1000;
  ...
  /**
   * Run `explore --json --sort installsAllTime --limit EXPLORE_LIMIT` and build
  ...
  private async refreshExploreCache(force = false): Promise<{
    ...
    const { stdout } = await execFileP(
      ...
      [
        ...
        'explore',
        '--json',
        '--limit', String(ClawHubSource.EXPLORE_LIMIT),
