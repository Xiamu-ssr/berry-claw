...
export const DEFAULT_AGENT_MCP_TEMPLATE = {
  mcpServers: {
    playwright: {
      command: 'npx',
      // Resolved from berry-claw's local node_modules (pinned dep in
      // package.json). Dropping `-y @latest` prevents concurrent agent
      // startups from racing on the same ~/.npm/_npx cache directory.
